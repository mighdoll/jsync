...
class PublishedRoots(partition:Partition) { 
  ...
  private def findPersistent(name:String):Option[Syncable] = {
    ...
    partition get nameToId(name) map {_ match { 
  ...
  /** 
   * We store PublishedRoot objects in the partition with a magic InstanceId 
   * so that we can refind them easily.  (With a key value store, we know we can index 
   * quickly by key).
   */
  private def nameToId(name:String):InstanceId = {
