...
$sync.observation = function() {
  ...
  var self = {
    ...
    /** all mutuations until the next pop() will be attributed to the server 
    * SOON get rid of this, just use withMutator() */
    serverMutator: function() {
      mutators.push("server");
    },
    
    /** return to previous mutator.  currently we never get more than 1 deep anyway
     * SOON get rid of this, just use withMutator() */
    popMutator: function() {
    ...
    /** call a function, all changes will be marked as originating from the mutator */
    ...
    /** watch for a particular property to change on an object or an array of objects */
     ...
     * every syncable and then propogate them to the server.  CONSIDER
     * this may send unnecessary changes to the server.  e.g. if an object is
     * modified and then discarded) */
